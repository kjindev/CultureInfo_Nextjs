...
export default function Map({ targetName }: PropsType) {
  ...
  const infoText = "서울시의 문화행사를 확인해보세요";
  const date = new Date();
  ...
  useEffect(() => {
    if (targetName === "search" && textRef.current?.innerHTML === "") {
      let i = 0;
      const textEffect = setInterval(() => {
        if (i < infoText.length && textRef.current) {
          let inputText = infoText[i];
          textRef.current.innerHTML += inputText;
          i++;
        } else if (i === infoText.length && textRef.current) {
          setEffectEnd(true);
  ...
  return (
    <div className="overflow-y-hidden bg-black relative text-white w-[100%] h-[100vh] flex justify-center">
      <div className="w-[100%] h-[100%] flex flex-col items-center">
        <div className="h-[5%] flex justify-center items-center">
          <span ref={textRef} className="text-xl"></span>
          <span
            className={
              effectEnd
                ? `blink text-xl text-violet-400`
        ...
        <div
          className="invisible flex w-[100%] h-[95%] pt-12 justify-center items-center"
          ref={effectRef}
        >
          <div
          ...
          <div className="flex flex-col w-[30%] h-[70%]">
            ...
            <div className="overflow-x-hidden" id="scroll">
              {cultureList.length === 0 ? (
                ...
                cultureList.map((item, index) => (
                  <div
                    key={index}
                    className="bg-white text-black hover:bg-sky-200 hover:cursor-pointer rounded-full m-3 p-2"
                  >
                    {item}
                  </div>
                ))
